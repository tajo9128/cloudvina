...
class ExportService:
    ...
    def export_jobs_csv(jobs: List[Dict]) -> StreamingResponse:
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=cloudvina_jobs_{datetime.now().strftime('%Y%m%d')}.csv"
            }
        )
    ...
    def export_jobs_json(jobs: List[Dict]) -> StreamingResponse:
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=cloudvina_jobs_{datetime.now().strftime('%Y%m%d')}.json"
            }
        )
    ...
    def export_job_pdf(job: Dict) -> StreamingResponse:
        ...
        # Title
        title = Paragraph(f"<b>CloudVina Docking Report</b>", styles['Title'])
        story.append(title)
        ...
        # Footer
        footer = Paragraph(
            f"<i>Generated by CloudVina on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</i>",
            styles['Normal']
        )
        story.append(footer)
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=cloudvina_job_{job.get('job_id', 'report')}_{datetime.now().strftime('%Y%m%d')}.pdf"
            }
        )
