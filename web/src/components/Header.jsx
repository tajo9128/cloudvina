        ...
        <header className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-3' : 'bg-transparent py-5'}`}>
            <div className="container mx-auto px-4 flex justify-between items-center">
                ...
                <Link to="/" className="flex items-center space-x-3 group">
                    ...
                    <h1 className={`text-xl font-bold tracking-tight ${scrolled ? 'text-slate-900' : 'text-white'} transition-colors`}>
                ...
                <nav className="hidden md:flex items-center space-x-5">
                    {navConfig.map((item) => (
                        <div key={item.name} className="relative group">
                            {item.dropdown ? (
                                <button
                                    className={`flex items-center gap-1 font-medium text-sm w-max uppercase tracking-wide transition-colors ${scrolled ? 'text-primary-600 hover:text-primary-800' : 'text-blue-300 hover:text-white'
                                        }`}
                                >
                                    {item.name}
                                    <ChevronDown size={14} />
                                ...
                                <Link
                                    to={item.path}
                                    className={`font-medium text-sm w-max uppercase tracking-wide transition-colors ${(location.pathname === item.path && item.name !== 'Home') ? 'text-white font-bold' :
                                            scrolled ? 'text-primary-600 hover:text-primary-800' : 'text-blue-300 hover:text-white'
                                        }`}
                                >
                                    {item.name}
                                </Link>
                                    ...
                                    <div className="py-2">
                                        {item.dropdown.map((subItem) => (
                                            <Link
                                                key={subItem.name}
                                                to={subItem.path}
                                                className="block px-4 py-2 text-sm text-primary-600 hover:bg-primary-50 hover:text-primary-800"
                                            >
                                                {subItem.name}
                                            </Link>
                                        ))}
