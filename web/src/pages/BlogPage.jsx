        ...
        <div className="min-h-screen bg-blue-mesh pt-24 pb-12">
            <div className="container mx-auto px-4">
                ...
                <div className="text-center mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-deep-navy-900 mb-4 tracking-tight">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                    ...
                    <div className="lg:col-span-2 space-y-10">
                            ...
                            {posts.map(post => (
                                <article key={post.id} className="bg-white rounded-2xl shadow-xl border border-gray-100 group hover:shadow-xl transition-all duration-300 flex flex-col h-full overflow-hidden">
                                    ...
                                    <div className="p-6 flex-1 flex flex-col">
                                        ...
                                        <h2 className="text-xl font-bold text-deep-navy-900 mb-3 group-hover:text-blue-600 transition-colors line-clamp-2">
                                        ...
                                        <Link
                                            to={`/blog/${post.id}`}
                                            className="inline-flex items-center text-sm font-semibold text-blue-600 hover:text-blue-700 transition-colors mt-auto"
                                        >
                                            Read Article
                        ...
                        <div className="flex justify-center space-x-2 mt-12">
                            <button className="w-10 h-10 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition">1</button>
                            <button className="w-10 h-10 rounded-lg bg-blue-600 text-white flex items-center justify-center shadow-lg shadow-blue-200">2</button>
                            <button className="w-10 h-10 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition">3</button>
                            <span className="w-10 h-10 flex items-center justify-center text-gray-400">...</span>
                            <button className="w-10 h-10 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-blue-50 hover:text-blue-600 transition">→</button>
                        </div>
                    </div>
                    ...
                    <aside className="lg:col-span-1 space-y-8">
                        ...
                        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-3 bg-white/50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition outline-none text-sm"
                                    value={searchQuery}
                                ...
                                {categories.map((cat, idx) => (
                                    <li key={idx}>
                                        <a href="#" className="flex items-center justify-between group">
                                            <span className="text-slate-600 group-hover:text-blue-600 transition-colors text-sm font-medium">{cat.name}</span>
                                            <span className="bg-blue-50 text-blue-600 py-0.5 px-2 rounded-full text-xs font-bold group-hover:bg-blue-100 transition-colors">{cat.count}</span>
                                        </a>
                                    </li>
                                ))}
                            </ul>
                            ...
                            <div className="space-y-6">
                                {posts.slice(0, 3).map(post => (
                                    <div key={post.id} className="flex space-x-4 group cursor-pointer">
                                        ...
                                        <div>
                                            <h4 className="text-sm font-bold text-deep-navy-900 leading-snug mb-1 group-hover:text-blue-600 transition-colors line-clamp-2">
                                                {post.title}
                                            </h4>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {tags.map((tag, idx) => (
                                    <a
                                        key={idx}
                                        href="#"
                                        className="text-xs font-medium bg-white border border-gray-200 text-slate-600 px-3 py-1.5 rounded-lg hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all"
                                    >
                                        #{tag}
                                    </a>
                                ))}
                        ...
                        {/* Newsletter Widget */}
                        <div className="bg-gradient-to-br from-blue-600 to-purple-600 rounded-2xl p-6 text-white shadow-lg shadow-blue-200">
                            <h3 className="text-lg font-bold mb-2">Subscribe to Newsletter</h3>
                            <p className="text-blue-100 text-sm mb-4">Get the latest updates and tutorials delivered to your inbox.</p>
                            <div className="space-y-3">
                                <input
                                    type="email"
                                    placeholder="Your email address"
                                    className="w-full px-4 py-2.5 rounded-lg bg-white/10 border border-white/20 text-white placeholder-blue-200 focus:outline-none focus:bg-white/20 transition text-sm"
                                />
