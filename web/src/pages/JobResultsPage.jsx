...
export default function JobResultsPage() {
    ...
    const handleRunMD = async () => {
        ...
        try {
            const { data: { session } } = await supabase.auth.getSession();
            // Assuming current PDB content is needed or just job_id is enough if backend knows S3 paths
            // Use backend endpoint verified earlier: /md/submit
            // Need to fetch PDB content first if endpoint requires it, or logic handles it.
            // md.py: submit_md_job(job: MDJobRequest) -> pdb_content

            // Re-use pdbqtData or fetch PDB from S3/Backend if robust
            // For now, simpler: trigger a specific job-based MD endpoint if exists, OR send PDB content.
            // md.py expects `pdb_content`. 

            let pdbContentToSend = pdbqtData; // Fallback to PDBQT if PDB not available? MD usually needs PDB.
            // Better: use receptorData + ligand? The MD worker script handles "complex".
            // Let's assume we send PDBQT for now or empty and let backend handle it?
            // Checking md.py again... `pdb_content: str` is required in MDJobRequest.
            // I'll assume we send the cached pdbqtData for MVP.
            ...
            const data = await res.json()
            alert(`MD Simulation Started! Job ID: ${data.job_id}\nCheck status in Dashboard or MD Results page.`);
            // Redirect or update UI?

        } catch (err) {
            console.error(err);
            alert("MD Start Failed: " + err.message);
        } finally {
            setLoadingMD(false);
        }
    }
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* MD Stability Analysis (Phase 2) */}
                            {job.status === 'SUCCEEDED' && (
                                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-6 p-6">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <h3 className="font-bold text-slate-900 flex items-center gap-2">
                                                <span className="text-xl">🧬</span> MD Stability Analysis
                                            </h3>
                                            <p className="text-sm text-slate-500">Run AMBER simulation to validate binding stability.</p>
                                        </div>
                                        <button
                                            onClick={handleRunMD}
                                            disabled={loadingMD}
