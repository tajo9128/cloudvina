...
import ReactApexChart from 'react-apexcharts'; // Using ApexCharts
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Active Jobs" value={stats.jobs?.running + stats.jobs?.queued} icon={Database} subtext={`${stats.jobs?.total} total today`} color="primary" trend="+12%" />
                <StatCard title="Success Rate" value={`${Math.round((stats.jobs?.completed / (stats.jobs?.total || 1)) * 100)}%`} icon={CheckCircle} subtext={`${stats.jobs?.failed} failed jobs`} color="green" />
                <StatCard title="Total Users" value={stats.users?.total} icon={Users} subtext="Lifetime registrations" color="purple" trend="+5 new" />
                <StatCard title="Server Load" value={`${stats.system?.cpu_percent}%`} icon={Cpu} subtext={`Mem: ${stats.system?.memory_percent}%`} color="orange" />
            </div>
        ...
        <div className="p-6 md:p-8 max-w-[1920px] mx-auto space-y-6">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex gap-3">
                    ...
                    <Link to="/admin/users" className="flex items-center gap-2 px-4 py-2 bg-secondary-600 text-white rounded-lg hover:bg-secondary-500 shadow-lg shadow-secondary-600/20 font-medium transition-all transform hover:-translate-y-0.5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {/* Right Col: System & Activity (1/3 width, stacked) */}
                <div className="space-y-6">
                    {/* Activity Feed */}
                    <div className="bg-slate-800/40 border border-primary-500/20 rounded-xl backdrop-blur-sm p-5 h-[400px] flex flex-col">
                        <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
